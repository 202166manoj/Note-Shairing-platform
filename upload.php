...
if (strlen($_SESSION['eid']) == 0) {
  ...
  if (isset($_POST['submit'])) {
    ...
    if (!in_array($extension, $allowed_extensions)) {
      echo "<script>alert('Invalid format. Only docs/doc/pdf allowed');</script>";
    } else {
      $file = md5($file) . time() . $extension;
      move_uploaded_file($_FILES["file"]["tmp_name"], "folder/" . $file);

      $sql = "INSERT INTO notes(UserID, Level, Subject, Module, Title, Description, File) 
              VALUES(:eid, :level, :subject, :module, :title, :description, :file)";
                ...
                <select name="module" required class="form-control">
                    ...
                    if ($query->rowCount() > 0) {
                        foreach ($results as $result) {
                            echo '<option value="'.htmlentities($result->SubjectName).'">'.htmlentities($result->SubjectName).'</option>';
                        }
                    }
                    ?>
                </select>
